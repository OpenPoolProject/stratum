...
impl MinerList {
    ...
    pub fn add_miner(&self, session_id: u32, miner: Miner) {
    ...
    #[must_use]
    pub fn remove_miner(&self, session_id: u32) -> Option<(u32, Miner)> {
        self.miners.remove(&(session_id))
    }

    #[must_use]
    pub fn get_miner_by_id(&self, session_id: u32) -> Option<Miner> {
        self.miners.view(&session_id, |_k, v| v.clone())
    }

    pub fn update_miner_by_session_id(&self, session_id: u32, miner: Miner) {
        self.miners.insert(session_id, miner);
    }
}
